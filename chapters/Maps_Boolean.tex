...
Conjugate Boolean networks need not have isomorphic wiring diagrams. In fact, it has recently been shown that every so-called non-trivial Boolean network is conjugate to one with a complete wiring diagram. It is well known that the bijections of $\F^n\to\F^n$ that preserving the wiring diagram are precisely the isometries of the Boolean cube. These form the group of signed permutations, also known as the type $B$ Weyl group, and they are isomorphic to the wreath product $C_2\wr C_n$. These can be considered as isomorphisms of Boolean networks. In this section, we will explore the concept of a homomorphism of Boolean networks. 

Summarize this chapter in a paragraph or two. 
...
\begin{proposition}\label{prop:semiconjugacy-fibers}
    Let $f\colon\F^n\to\F^n$ and let $h\colon\F^n\to\F^m$ be surjective. There is a Boolean network $g$ satisfying~\eqref{eq:semiconjugacy} if and only if
...
\begin{proof}
    Necessity follows by applying $g$ to $h(x)=h(x')$. Conversely, for $y\in\F^m$, choose $x\in h^{-1}(y)$ and put $g(y)=h(f(x))$. Condition~\eqref{eq:forward-compatible-state-fibers} makes this definition independent of the choice of $x$. It gives $gh=hf$, and surjectivity of $h$ forces these values of $g$ at every $y$.
\end{proof}
...
For Boolean self-maps, this criterion says that changing one input affects at most one output at any fixed state. It is the local out-degree characterization of nonexpansiveness in \cite[Proposition~2]{richard2011nonexpansive}. The proof above applies also when the two cubes have different dimensions. Compositions of contractions are contractions. A surjective contraction from $\F^n$ to itself is an isometry. Indeed, it is a bijection of a finite set, so $h^N=\id$ for some $N\ge1$. Repeated application of~\eqref{eq:hamming-contraction} gives
\[
...
\begin{proof}
    ...
    Conversely, under (iii), changing an input coordinate $i$ can affect only the output with $a=\phi(i)$, which gives (i). Finally, equation~\eqref{eq:compatible-node-directions} implies~\eqref{eq:contraction-edges}, so Lemma~\ref{lem:contraction-edges} gives contraction.
\end{proof}
