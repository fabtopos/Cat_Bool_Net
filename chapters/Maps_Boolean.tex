...
\begin{lemma}\label{lem:isometry-directions}
    ...
    \begin{equation}\label{eq:isometry-directions}
        h(x\oplus e_i)=h(x)\oplus e_{\sigma(i)}
...
\begin{proof}
    ...
    \[
        h(x\oplus e_i)=h(x)\oplus e_{\sigma_x(i)}.
    \]
    Traversing the same edge in the opposite direction shows that
    $\sigma_{x\oplus e_i}(i)=\sigma_x(i)$.

    Now let $i\ne j$. The four distinct states
    \[
        x,\quad x\oplus e_i,\quad x\oplus e_i\oplus e_j,
        \quad x\oplus e_j
    \]
    form a square in $Q_n$. Put $z=h(x)$, $r=\sigma_x(i)$, and $s=\sigma_x(j)$. The two common neighbors of $z\oplus e_r$ and $z\oplus e_s$ are precisely $z$ and $z\oplus e_r\oplus e_s$. Since $h$ is injective, the vertex opposite $x$ must therefore satisfy
    \[
        h(x\oplus e_i\oplus e_j)=z\oplus e_r\oplus e_s.
    \]
    Comparing this with $h(x\oplus e_j)=z\oplus e_s$ gives
    $\sigma_{x\oplus e_j}(i)=\sigma_x(i)$. Consequently, $\sigma_x(i)$ is unchanged when any coordinate of $x$ is changed. The cube is connected, so $\sigma_x$ is a single permutation $\sigma$, independent of $x$. Its uniqueness follows from the images of the edges incident with $\0$.
...
\begin{example}\label{ex:isometric-wiring}
    ...
    \[
        f(x_1,x_2,x_3)=(x_1x_2,\ x_1\vee x_3,\ x_1\oplus x_2)
    \]
    ...
    \begin{align*}
        ...
        &=h\bigl(y_2\Not{y}_3,\ y_2\vee\Not{y}_1,
                 \ y_2\oplus\Not{y}_3\bigr)\\
        &=\bigl(y_2\oplus y_3,\ y_2\Not{y}_3,
                 \ y_1\Not{y}_2\bigr).
    \end{align*}
...
\]
Thus an input coordinate $i$ is moved to output coordinate $\sigma(i)$, consistently with the relabeling of the wiring diagram above. For $a\in\F^n$, define $\tau_a(x)=a\oplus x$. This map complements precisely those coordinates $j$ for which $a_j=1$.
...
\begin{proof}
    ...
    \[
        h(x)=a\oplus\bigoplus_{\{i:x_i=1\}}e_{\sigma(i)}
             =a\oplus P_\sigma(x).
    \]
    Here the empty sum is $\0$. Evaluating at $\0$ determines $a$, and evaluating at each $e_i$ then determines $\sigma$, proving uniqueness.

    Conversely, suppose that $h(x)=a\oplus P_\sigma(x)$. For every $x,y\in\F^n$,
    \begin{align*}
        d_H\bigl(h(x),h(y)\bigr)
        &=\bigl|\{j:a_j\oplus x_{\sigma^{-1}(j)}
                     \ne a_j\oplus y_{\sigma^{-1}(j)}\}\bigr|\\
        &=\bigl|\{j:x_{\sigma^{-1}(j)}\ne y_{\sigma^{-1}(j)}\}\bigr|\\
        &=d_H(x,y).
    \end{align*}
...
\[
    g_j(y)=a_j\oplus
    f_{\sigma^{-1}(j)}\bigl(P_{\sigma^{-1}}(y\oplus a)\bigr).
\]
...
\begin{example}\label{ex:recover-isometry}
    ...
    Thus $a=(1,0,1)$, and
    \[
        h(e_1)\oplus a=e_2,\qquad
        h(e_2)\oplus a=e_3,\qquad
        h(e_3)\oplus a=e_1.
    \]
    We recover $\sigma=(1\ 2\ 3)$ and
    \[
        h(x)=(1,0,1)\oplus(x_3,x_1,x_2),
    \]
...
\begin{proof}
    ...
    $f_{\sigma(i)}(x)=a_{\sigma(i)}\oplus x_i$. Its wiring diagram therefore has exactly the edges $i\longrightarrow\sigma(i)$, so every vertex has one incoming and one outgoing edge.
    ...
    $f_{\sigma(i)}(x)=a_{\sigma(i)}\oplus x_i$ for some $a_{\sigma(i)}\in\F$, and Theorem~\ref{thm:cube-isometries} applies.
\end{proof}
